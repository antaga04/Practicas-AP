...
l * Img2->nChannels);
        ...
        for (cc = 0; cc < ANCHO * Img1->nChannels; cc += 16) {
            ...
            C = _mm_sad_epu8(A, B); // hace la suma y deja dos datos colocados en r1 y 
r3 (vacio dato vacio dato)
            D = _mm_srli_si128(C, 8); // en D se guarda en la esquina de la derecha el 
dato C
            E = _mm_add_epi32(C, D); // se suman el dato D y C y se convierte a entero 
de 32 bits
